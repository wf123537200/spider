...
const Comp: React.FC<PropsType> = ({
  ...
  return (
    <>
      <Alert
        message="Request异步请求实例，项目http请求使用axios实现，在src/config/api中对请求URL进行模块化管理，在service与request的js文件中对axios进行二次封装，在axios原有提供的参数上，增加了路径参数(例如: /api/url/:id，其中id为路径参数)与一些其他控制参数(例如noGlobalError)，以下为一些常见的异步请求例子。具体写法请参考代码，models/examples.ts"
        type="info"
        closable
        showIcon
